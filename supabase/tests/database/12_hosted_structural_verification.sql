...
-- correction-pass entry for why this replaced a standalone
...
-- catching both "missing" and "unexpected" in either direction.
select tables_are(
  'public',
  array[
...
-- 2. RLS enabled on every one of those tables, no exceptions
-- (docs/DATABASE_SCHEMA.md, docs/ARCHITECTURE.md §8). One assertion per
-- table, generated from pg_tables rather than hardcoded, so a table
-- renamed without updating this file still gets checked by name.
select ok(rowsecurity, format('RLS enabled on public.%s', tablename))
from pg_tables
...
-- 4. service_role has table privileges on every table (Phase 2A
-- correction pass, docs/DECISIONS.md 2026-07-24 entry — bypassrls alone is
-- not sufficient; 20260723091900_service_role_grants.sql must be
-- deployed).
select ok(
  has_table_privilege('service_role', format('public.%I', tablename), 'SELECT'),
  format('service_role has SELECT privilege on public.%s', tablename)
)
from pg_tables
where schemaname = 'public'
order by tablename;
...
-- decision: stricter than RLS alone — docs/DECISIONS.md).
select is(
  (
    select count(*)::int
